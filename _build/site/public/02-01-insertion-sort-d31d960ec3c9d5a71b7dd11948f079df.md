...
Using @fig-2-2 as a model, illustrate the operation of {sc}`Insertion-Sort` on an array initially containing the sequence $\langle 31, 41, 59, 26, 41, 58 \rangle$.

### 2.1-2
Consider the procedure {sc}`Sum-Array` below. It computes the sum of the $n$ numbers in array $A[1:n]$. State a loop invariant for this procedure, and use its initialization, maintenance, and termination properties to show that the {sc}`Sum-Array` procedure returns the sum of the numbers in $A[1:n]$.
...
Rewrite the {sc}`Insertion-Sort` procedure to sort into monotonically decreasing instead of monotonically increasing order.
...
* **Output:** An index $i$ such that $x$ equals $A[i]$ or the special value {sc}`nil` if $x$ does not appear in $A$.

Write pseudocode for **linear search**, which scans through the array from beginning to end, looking for $x$. Using a loop invariant, prove that your algorithm is correct. Make sure that your loop invariant fulfills the three necessary properties.

### 2.1-5

Consider the problem of adding two n-bit binary integers $a$ and $b$, stored in two $n$-element arrays $A[0:n-1]$ and $B[0:n-1]$, where each element is either 0 or 1, $a = \sum_{i=0}^{n-1}A[i]\cdot 2^i$, and $b = \sum_{i=0}^{n-1}B[i]\cdot 2^i$.

The sum $c = a + b$ of the two integers should be stored in binary form in an $(n + 1)$-element array $C[0:n]$, where $c = \sum_{i=0}^{n}C[i] \cdot 2^i$.

Write a procedure {sc}`Add-Binary-Integers` that takes as input arrays $A$ and $B$, along with the length $n$, and returns array $C$ holding the sum.
