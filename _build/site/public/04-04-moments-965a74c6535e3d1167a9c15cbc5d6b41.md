...
## Example 4.4.1: A Symmetric pdf

Suppose that X has a continuous distribution for which the p.d.f.
has the following form:

We shall determine the mean of X and all the central moments.
It can be shown that for every positive integer k,

Hence, all the moments of X exist.

Furthermore, since f (x) is symmetric with respect
...
In Example 4.4.1, we saw that the odd central moments are all 0 for a
...
## Definition 4.4.1: Skewness

Let X be a random variable with mean μ, standard deviation σ, and finite
...
## Example 4.4.2: Skewness of Binomial Distributions

Let X have the binomial distribution with parameters
...
## Definition 4.4.2: Moment Generating Function
...
## Note: The Moment Generating Function of X Depends Only on the Distribution of $X$.

Since the m.g.f. is the expected value of a function of X, it must depend only
...
## Example 4.4.3: Calculating an MGF

Suppose that X is a random variable for which the p.d.f. is as
follows:
...
Therefore, E(X) = ψ(0) = 1 and E(X2) = ψ(0) = 2. It now follows that
Var(X) = ψ
  
(sec-4-4-3)=
...
## Example 4.4.4: Calculating the m.g.f. of a Linear Function

Suppose that the distribution of X is as specified in @exm-4-4-3. We saw that the m.g.f. of X for t < 1 is

ψ1(t) = 11− t

If Y = 3 − 2X, then the m.g.f. of Y is finite for t > −1/2 and will have the value
ψ2(t) = e3tψ1(−2t) = e3t1+ 2t
...
Since the random variables X1, . . . ,Xn are independent, it follows from @thm-4-2-6 that

# The Moment Generating Function for the Binomial Distribution

Suppose that a random variable X has the binomial distribution with parameters n and p. In
...
It follows from @thm-4-4-4 that the m.g.f. of X in this case is ψ(t) = (pet + 1− p)n. (4.4.4)

# Uniqueness of Moment Generating Functions

We shall now state one more important
...
Moment generating functions provide a simple way to derive the distribution of the sum of two independent binomial random variables with the same second parameter.
...
Thus, ψ(0) = E(X), ψ(0) = E(X2), ψ(0) = E(X3), and so on.
...
If the $k$th moment of a random variable exists, then so does the $j$th moment for every $j < k$. The moment generating function of $X$, $\psi(t) = \mathbb{E}[e^{tX}]$, if it is finite for $t$ in a neighborhood of 0, can be used to find moments of $X$. The $k$th derivative of $\psi(t)$ at $t = 0$ is $\mathbb{E}[X^k]$. The m.g.f. characterizes the distribution in the sense that all random variables that have the same m.g.f. have the same distribution.
