...
* (b) Write pseudocode to implement the naive polynomial-evaluation algorithm that computes each term of the polynomial from scratch. What is the running time of this algorithm? How does it compare with {sc}`Horner`?
* (c) Consider the following loop invariant for the procedure {sc}`Horner`:
